...
//        );
    ...
    public List<ProjectData> findAllCriteria(ProjectSearchRequest request) {
        CriteriaBuilder criteriaBuilder = em.getCriteriaBuilder();
        CriteriaQuery<ProjectData> criteriaQuery = criteriaBuilder.createQuery(ProjectData.class);
        ...
        if (request.getTotalDeposit() != 0) {
            predicates.add(criteriaBuilder.equal(root.get("totalDeposit"), request.getTotalDeposit()));
        }
        if (request.getGarage() != 0) {
            predicates.add(criteriaBuilder.equal(root.get("garage"), request.getGarage()));
        }
        ...
        if (request.getPropClosingYear() != 0) {
            predicates.add(criteriaBuilder.equal(root.get("propClosingYear"), request.getPropClosingYear()));
        }
        ...
        if (request.getForntLotSize() != 0) {
            predicates.add(criteriaBuilder.equal(root.get("frontLotSize"), request.getForntLotSize()));
        }
