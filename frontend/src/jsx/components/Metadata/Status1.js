...
function Status1() {
  ...
  const handleNewData = (index) => {
    setText('')
    const newData = {
      status: (metadata[index]?.type || "").trim(),
    };
  ...
  const handleSave = (index) => {
    setNewType(false);
    const currentValue = metadata[index].status;
    const updatedData = {
      id: metadata[index].id,
      status: (metadata[index]?.type || "").trim(), // Trim whitespace if type exists
    };
  ...
  const handleConfirmDelete = () => {
    ...
    axios.delete(`http://localhost:8086/api/metadata/status/delete/${garageIdToDelete}`)
      .then((response) => {
            ...
            <tr key={index}>
              ...
              <td className="text-center w-40 ">
                {editingIndex === index ? (
                  ...
                  <span>{metadata[index].status}</span>
                )}
              </td>
